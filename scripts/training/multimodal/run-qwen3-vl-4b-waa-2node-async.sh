...
# Qwen3-VL-4B + WindowsAgentArena, fully asynchronous, 2-node/8-GPU correctness smoke.
...
ckpt_args=(
    ...
    --save "${SAVE_DIR}/Qwen3-VL-4B-WAA-Checkpoint"
    --save-interval 100
...
resource_args=(
    # actor 4 GPUs on node 0 (TP=1, DP=4), rollout 4 TP=1 engines on node 1.
    # rollout must stay within one node (rendezvous address assumption).
...
log_args=(
    ...
    --tb-experiment-name "${EXP_NAME:-qwen3-vl-4b-waa-async-${timestamp}}"
)
