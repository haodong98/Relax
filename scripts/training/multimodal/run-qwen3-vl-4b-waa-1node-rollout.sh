...
# Qwen3-VL-4B WAA rollout-only smoke on one four-GPU node.
...
ray job submit ${RAY_NO_WAIT:+--no-wait} --address="http://127.0.0.1:8265" \
    ...
    --agent-cwd "${agent_dir}" \
    --agent-env "WAA_BROKER_MANIFEST_DIR=${WAA_BROKER_MANIFEST_DIR}" \
        "WAA_BROKER_TOKEN_FILE=${WAA_BROKER_TOKEN_FILE}" \
        "WAA_MAX_STEPS=${WAA_MAX_STEPS:-8}" \
        "WAA_LEASE_WAIT_S=${WAA_LEASE_WAIT_S:-600}" \
        "WAA_CHAT_TIMEOUT_S=${WAA_CHAT_TIMEOUT_S:-900}" \
    --agent-timeout 1200 \
    --num-rollout "${NUM_ROLLOUT:-1}" --rollout-batch-size 1 \
    --n-samples-per-prompt 1 --global-batch-size 1 \
    --rollout-max-prompt-len 16384 --rollout-max-response-len 512 \
    --rollout-max-context-len 32768 --rollout-temperature 1 \
    --rollout-seed "${SEED:-42}" --seed "${SEED:-42}" \
    --debug-rollout-only --save-debug-rollout-data "${EXP_DIR}/debug_rollout/{rollout_id}.pt" \
    --num-data-storage-units 1 --fully-async \
    ...
    --tb-project-name "Relax/dev/waa-rollout" \
    --tb-experiment-name "qwen3-vl-4b-waa-rollout-${timestamp}"
