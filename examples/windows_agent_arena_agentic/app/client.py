...
"""Relax managed-command client for one WindowsAgentArena episode."""
...
def _post_json(url: str, payload: dict[str, Any], *, token: str, timeout: float) -> dict[str, Any]:
    ...
    try:
        ...
        raise RemoteError(f"HTTP {exc.code} from remote service", status=exc.code) from exc
    except (urllib.error.URLError, TimeoutError, json.JSONDecodeError) as exc:
        raise RemoteError("remote service unavailable") from exc
...
def _metadata(session_input: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(metadata, dict):
        raise ValueError("session input metadata is missing")
    required = ("task_id", "domain", "task_manifest_digest")
...
def _png_dimensions(screenshot: str) -> tuple[int, int]:
    ...
    if width <= 0 or height <= 0:
        raise RuntimeError("broker returned invalid PNG dimensions")
    return width, height
    ...
    return [
        {"role": "system", "content": ACTION_SYSTEM_PROMPT},
        {
            ...
            "content": [
                {
                    "type": "text",
                    "text": f"Task:\n{instruction}\n\nObservation 0. Screenshot dimensions: {width}x{height}.",
                },
...
def observation_message(screenshot: str, *, turn: int, status: str, width: int, height: int) -> dict[str, Any]:
    return {
        ...
        "content": [
            {
                "type": "text",
                "text": f"Observation {turn}. Previous action status: {status}. Screenshot dimensions: {width}x{height}.",
            },
            _image_part(screenshot),
        ],
    }
...
def _load_brokers(manifest_dir: Path) -> list[str]:
    ...
    for path in sorted(manifest_dir.glob("broker-*.json")):
        try:
            ...
            continue
        if payload.get("schema_version") == "waa.broker_manifest.v1" and isinstance(payload.get("broker_url"), str):
...
def acquire_broker(
    ...
    *,
    ...
    session_rng = random.Random(os.environ.get("RELAX_SESSION_ID", ""))
    while time.monotonic() < deadline:
        brokers = _load_brokers(manifest_dir)
        session_rng.shuffle(brokers)
        for broker in brokers:
            ...
            try:
                ...
                continue
        time.sleep(1.0)
    raise TimeoutError("timed out waiting for a WAA broker lease")
...
def run_episode(session_input: dict[str, Any]) -> dict[str, Any]:
    ...
    manifest_dir = Path(os.environ["WAA_BROKER_MANIFEST_DIR"])
    token = Path(os.environ["WAA_BROKER_TOKEN_FILE"]).read_text(encoding="utf-8").strip()
    session_id = os.environ["RELAX_SESSION_ID"]
    relax_base_url = os.environ["RELAX_BASE_URL"]
    max_steps = int(os.environ.get("WAA_MAX_STEPS", "8"))
    lease_wait = float(os.environ.get("WAA_LEASE_WAIT_S", "1200"))
    chat_timeout = float(os.environ.get("WAA_CHAT_TIMEOUT_S", "1800"))
    if not 1 <= max_steps <= 32:
        raise ValueError("WAA_MAX_STEPS must be in [1,32]")
    ...
    try:
        ...
        for turn in range(1, max_steps + 1):
            ...
            if _png_dimensions(screenshot) != (width, height):
                raise RuntimeError("broker screenshot dimensions changed during the episode")
            if action.terminal:
        ...
        if broker is not None and lease is not None:
            ...
            # request_id is known before /v1/lease starts. A cancellation HTTP
            # handler can therefore clean up after a cold-start caller that is
            # SIGKILLed during the managed runner's five-second grace period.
            try:
                _post_json(
                    broker_attempt + "/v1/cancel",
                    {"request_id": request_id},
                    token=token,
                    timeout=360.0,
                )
...
def main() -> None:
    def terminate(signum: int, _: Any) -> None:
        # Managed runners allow five seconds between SIGTERM and SIGKILL. Raise
        # into run_episode so its finally block immediately sends broker release.
        raise InterruptedError(f"managed WAA client received signal {signum}")
    ...
    session_input = json.loads(args.input_json.read_text(encoding="utf-8"))
    output = run_episode(session_input)
