...
# 2 nodes x 4 GPUs/node x 2 hours = 16 GPU-hours, WAA Qwen3-VL-4B correctness smoke.

#SBATCH --account=infra01
#SBATCH --partition=normal
#SBATCH --qos=normal
#SBATCH --job-name=relax-waa-4b-2node
...
if [ "${SLURM_JOB_NUM_NODES:-0}" -ne 2 ]; then
    echo "ERROR: this correctness smoke requires exactly two nodes" >&2
    exit 1
fi
...
srun --nodes=2 --ntasks=2 --ntasks-per-node=1 --cpus-per-task=4 --overlap --exact \
...
cleanup() {
    ...
    if [ -n "${broker_step_pid}" ]; then
        ...
        if ! srun --nodes=2 --ntasks=2 --ntasks-per-node=1 --overlap --exact \
...
/usr/bin/python3.11 "${script_dir}/scripts/wait_brokers.py" \
    --manifest-dir "${WAA_BROKER_MANIFEST_DIR}" --expected 2 --timeout 300

srun --nodes=2 --ntasks=2 --ntasks-per-node=1 --overlap --exact \
    --environment="${EDF_TOML}" \
    python3 "${script_dir}/scripts/wait_brokers.py" \
    --manifest-dir "${WAA_BROKER_MANIFEST_DIR}" --expected 2 --timeout 30
...
if [ "${NUM_ROLLOUT}" -ne 3 ]; then
    echo "ERROR: correctness smoke requires exactly three rollout rounds" >&2
    exit 1
fi
export WAA_MAX_STEPS="${WAA_MAX_STEPS:-8}"
export WAA_PROMPT_DATA="${DATA_DIR}/waa/smoke_train.jsonl"
export RUN_SCRIPT="${RELAX_REPO_DIR}/scripts/training/multimodal/run-qwen3-vl-4b-waa-2node-async.sh"
unset RAY_NO_WAIT
