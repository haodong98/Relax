...
# 4 nodes x 4 GPUs/node x 2 hours = 32 GPU-hours (within the 80 GPU-hour cap).

#SBATCH --account=infra01
#SBATCH --partition=normal
#SBATCH --qos=normal
#SBATCH --job-name=relax-waa-2b
...
if [ "${SLURM_JOB_NUM_NODES:-0}" -ne 4 ]; then
    echo "ERROR: this functional smoke requires exactly four nodes" >&2
    exit 1
fi
...
for required_path in \
    ...
    "${VENV_BIN}" \
    "${CUDNN_LIB_DIR}" \
    "${MODEL_DIR}/Qwen3-VL-2B-Instruct"; do
...
srun --nodes=4 --ntasks=4 --ntasks-per-node=1 --cpus-per-task=4 --overlap --exact \
    ...
    --model "${MODEL_DIR}/Qwen3-VL-2B-Instruct" --output-dir "${EXP_DIR}/training_env_preflight"
...
cleanup() {
    ...
    if [ -n "${broker_step_pid}" ]; then
        ...
        if ! srun --nodes=4 --ntasks=4 --ntasks-per-node=1 --overlap --exact \
...
/usr/bin/python3.11 "${script_dir}/scripts/wait_brokers.py" \
    --manifest-dir "${WAA_BROKER_MANIFEST_DIR}" --expected 4 --timeout 300

# This second probe runs inside every EDF container. It is a hard all-to-all
# network gate before GPUs are assigned to Relax services.
srun --nodes=4 --ntasks=4 --ntasks-per-node=1 --overlap --exact \
    --environment="${EDF_TOML}" \
    python3 "${script_dir}/scripts/wait_brokers.py" \
    --manifest-dir "${WAA_BROKER_MANIFEST_DIR}" --expected 4 --timeout 30
...
export RUN_SCRIPT="${RELAX_REPO_DIR}/scripts/training/multimodal/run-qwen3-vl-2b-waa-4node-async.sh"
unset RAY_NO_WAIT
...
# SPMD performs broad stale-process cleanup before starting Ray. Probe again
# after that window so a pre-cleanup broker check cannot be a false positive.
sleep 15
/usr/bin/python3.11 "${script_dir}/scripts/wait_brokers.py" \
    --manifest-dir "${WAA_BROKER_MANIFEST_DIR}" --expected 4 --timeout 30
